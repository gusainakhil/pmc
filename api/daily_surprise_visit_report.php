...
if ($data && isset($data["pageIds"], $data["values"])) {
    ...
    $tokenid = $data['token']; // unique alphanumeric string based on current time
...
$query = "INSERT INTO baris_survey 
    ...
    foreach ($pageIds as $index => $pageId) {
        ...
        $valueText = strtolower($values[$index]); // to lower to match keys
        $surveyValue = isset($valueMap[$valueText]) ? $valueMap[$valueText] : 0; // default 0 if not matched
    ...
    if ($successCount == count($pageIds)) {
        ...
        $response["message"] = "All survey rows inserted successfully in baris_survey.";
    } else {
