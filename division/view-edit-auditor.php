...
// Set or update session for DivisionId
if (isset($_GET['divisionid']) && $_GET['divisionid'] !== '') {
    $_SESSION['DivisionId'] = $_GET['divisionid'];
...
// $divisionId = $_SESSION['DivisionId'];
 $divisionId  = 30;
$whereClause = "WHERE db_usertype = 'auditor' AND reportType = 'PMC'";
if (!empty($divisionId)) {
    $whereClause .= " AND divisionid = '" . $conn->real_escape_string($divisionId) . "'";
}
if (!empty($searchTerm)) {
...
// Fetch Auditor users with pagination
$query = "SELECT userId, db_userLoginName, db_username, db_email, db_phone, db_designation, OrgName, db_valid_from, db_valid, LastLogin 
...
<head>
    ...
    <title>Auditor Management | Station Cleaning</title>
    ...
    <div class="ml-0 md:ml-72 flex-1">
        ...
        <header class="bg-white shadow-sm sticky top-0 z-10">
            <div class="flex items-center justify-between px-6 py-4">
                <div class="flex items-center">
                    ...
                    <h1 class="text-xl font-bold text-gray-800">Auditor Management</h1>
                </div>
                
                <div class="flex items-center space-x-4">
                    <a href="create-auditor.php" class="flex items-center text-sm font-medium text-blue-600 hover:text-blue-800">
        ...
        <div class="p-6">
                    ...
                    <ol class="inline-flex items-center space-x-1 md:space-x-3">
                        ...
                        <li aria-current="page">
                            <div class="flex items-center">
                                ...
                                <span class="text-gray-700">Auditor Users</span>
                            </div>
                        </li>
                    </ol>
                </nav>
                ...
                <div class="p-6 border-b border-gray-200">
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                        <div class="mb-4 md:mb-0">
                            <h2 class="text-lg font-semibold text-gray-800">Auditors</h2>
                            <p class="text-gray-500 text-sm mt-1">Manage Auditor users and their station assignments</p>
                        </div>
                        ...
                        <div class="flex flex-col sm:flex-row gap-3">
                            <form action="" method="GET" class="flex items-center gap-2">
                                <input 
                                    type="text" 
                                    name="search" 
                                    placeholder="Search Auditor users..." 
                                ...
                                >
                                <!-- <select name="divisionid" class="border border-gray-300 px-2 py-2 rounded focus:outline-none focus:ring-2 focus:ring-blue-300">
                                    <option value="">All Divisions</option>
                                    <option value="30" <?php if(isset($_GET['divisionid']) && $_GET['divisionid']=="30") echo 'selected'; ?>>Division 30</option>
                                </select>
                                <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded-r-lg hover:bg-blue-600 flex items-center">
                                    <i class="fas fa-search mr-1"></i> Search
                                </button> -->
                            </form>
                            <a href="create-auditor.php" class="flex items-center justify-center bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-lg">
                                <i class="fas fa-plus mr-2"></i>
                                Create Auditor
                            </a>
                        </div>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200"></tbody>
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="relative inline-block text-left">
                                        <button type="button" class="inline-flex justify-center w-full rounded-md border border-gray-300 shadow-sm px-2 py-1 bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none" id="menu-button-<?php echo $row['userId']; ?>" aria-expanded="true" aria-haspopup="true">
                                            <i class="fas fa-ellipsis-v"></i>
                                        </button>
                                        <div class="origin-top-right absolute right-0 mt-2 w-40 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none z-10 hidden group-hover:block" role="menu" aria-orientation="vertical" aria-labelledby="menu-button-<?php echo $row['userId']; ?>">
                                            <a href="edit-auditor.php?id=<?php echo $row['userId']; ?>" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" role="menuitem">
                                                <i class="fas fa-edit mr-2 text-blue-500"></i> Edit
                                            </a>
                                            <a href="#" onclick="confirmDelete(<?php echo $row['userId']; ?>, '<?php echo addslashes($row['db_userLoginName']); ?>')" class="block px-4 py-2 text-sm text-red-600 hover:bg-red-50" role="menuitem">
                            ...
                            <tr>
                                <td colspan="7" class="px-6 py-10 text-center text-gray-500">
                                    ...
                                    <p>No Auditor users found<?php echo !empty($searchTerm) ? ' matching "' . htmlspecialchars($searchTerm) . '"' : ''; ?></p>
                                    <?php if (!empty($searchTerm)): ?>
                                        <a href="view-edit-auditor.php" class="text-blue-500 hover:underline mt-2 inline-block">Clear search</a>
                                    <?php endif; ?>
                                </td>
                            </tr>
                            <?php
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                <a href="create-auditor.php" class="card p-5 hover:shadow-md transition-shadow">
                    <div class="flex items-center mb-4">
                        <div class="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center mr-3">
                            <i class="fas fa-user-plus text-blue-600"></i>
                        </div>
                        <h3 class="font-semibold">Add New Auditor</h3>
                    </div>
                    <p class="text-sm text-gray-600 mb-4">
                        Create a new Auditor account for station management.
                    </p>
                    <span class="text-sm text-blue-600 font-medium hover:text-blue-800 flex items-center">
                        <i class="fas fa-plus mr-2"></i>
                        Create Auditor
                    </span>
                </a>
                
                <a href="#" class="card p-5 hover:shadow-md transition-shadow">
                    <div class="flex items-center mb-4">
                        <div class="w-10 h-10 rounded-full bg-green-100 flex items-center justify-center mr-3">
                            <i class="fas fa-file-export text-green-600"></i>
                        </div>
                        <h3 class="font-semibold">Export Auditor List</h3>
                    </div>
                    <p class="text-sm text-gray-600 mb-4">
                        Download a spreadsheet of all Auditors.
                    </p>
                    <span class="text-sm text-green-600 font-medium hover:text-green-800 flex items-center">
                ...
                <a href="#" class="card p-5 hover:shadow-md transition-shadow">
                    <div class="flex items-center mb-4">
                        ...
                        <h3 class="font-semibold">Auditor Analytics</h3>
                    </div>
                    <p class="text-sm text-gray-600 mb-4">
                        View activity reports and performance metrics for Auditors.
                    </p>
                    <span class="text-sm text-purple-600 font-medium hover:text-purple-800 flex items-center">
...
<script>
    ...
    function confirmDelete(userId, userName) {
        ...
        confirmDeleteBtn.href = `delete-auditor.php?userId=${userId}`;
    ...
    // Status message handling from URL parameters
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (status === 'password-reset') {
            alert('Password has been reset successfully. Please provide the new password to the user.');
        } else if (status === 'activated') {
            alert('User account has been activated successfully.');
        } else if (status === 'deactivated') {
            alert('User account has been deactivated successfully.');
        } else if (error) {
            alert('An error occurred: ' + error.replace('-', ' '));
        }
    });
</script>
